...
def train():
    ...
    re_id_net_1 = Model(input=[base_model.input], output=[base_model_1.output])
    re_id_net_2 = Model(input=[base_model.input], output=[base_model_2.output])
    for layer in re_id_net.layers:
        layer.trainable = True
    re_id_net.load_weights('basic_train_on_market.ckpt', by_name=True)
    re_id_net_1.load_weights('basic_train_on_market.ckpt', by_name=True)
    re_id_net_2.load_weights('basic_train_on_market.ckpt', by_name=True)
    ...
    vgg_id_loss_1 = tl.cost.mean_squared_error(net_g_4_output_vgg.outputs, net_image_2_vgg.outputs, is_mean=True)
    vgg_id_loss_2 = tl.cost.mean_squared_error(net_g_8_output_vgg.outputs, net_image_4_vgg.outputs, is_mean=True)
    vgg_id_loss=vgg_id_loss+vgg_id_loss_1+vgg_id_loss_2
    ...
    re_id_net_1.compile(optimizer=SGD(lr=0.001, momentum=0.9), loss={'id_preds': 'categorical_crossentropy'})
    re_id_net_2.compile(optimizer=SGD(lr=0.001, momentum=0.9), loss={'id_preds': 'categorical_crossentropy'})
    ...
    ###============================= LOAD VGG ===============================###
    vgg19_npy_path = "F:/anaconda/Lib/site-packages/keras/models/vgg19.npy"
